## For: ACIP analyses using LHS parameter sampling + including indirect effects
...
## Options: only indirect protection against transmission ('trans')
## Effect against transmission (indirect_trans) = 10 % of the effect against symptoms
## Effect against infection    (indirect_inf)   = 0 %

## Or: only indirect protection against infection ('inf')
## Effect against transmission (indirect_trans) = 0 % of the effect against symptoms
## Effect against infection    (indirect_inf)   = 10 %

## Or: indirect protection against infection and transmission ('both')
## Effect against transmission (indirect_trans) = 10 % of the effect against symptoms
## Effect against infection    (indirect_inf)   = 10 %

datestamp <- "_trans_220707"  # or "_inf_xxxxxx"  or "_both_xxxxxx"
...
## Parameters: add indirect effects --------------------------------------------

## Transmission
pars$value[pars$name == "VE_trans"]   <- indirect_trans
pars$value[pars$name == "hdVE_trans"] <- indirect_trans

## Infection
pars$value[pars$name == "VE_inf"]   <- indirect_inf
pars$value[pars$name == "hdVE_inf"] <- indirect_inf
...
save(all, file = paste0("results/LHS_indirect_array", array_id, "of", tot_array, "_", datestamp, ".RData"))
