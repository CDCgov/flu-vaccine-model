...
## with: logit-normal distribution for enhanced vaccine VE, rather than uniform
...
get_grid <- function(season, parmat0, pars_min, pars_max, pargrid, nsim, nfixed) {
  
  parmat <- t( apply(parmat0, 1, function(x) qunif(x, as.numeric(pars_min), as.numeric(pars_max))))
  
  if (season == "low") {
    parmat[,"hdVE_mult"] <- 1 + gamlss.dist::qLOGITNO(parmat0[,"hdVE_mult"], mu = 0.225, sigma = 0.50)
  } else {
    parmat[,"hdVE_mult"] <- 1 + gamlss.dist::qLOGITNO(parmat0[,"hdVE_mult"], mu = 0.450, sigma = 0.75)
  }
...
parmat0 <- randomLHS(nsim, npars)
colnames(parmat0) <- names(pars_min_low)

# Different grids for different seasons
parmat_low  <- get_grid("low",  parmat0, pars_min_low,  pars_max_low,  pargrid, nsim, nfixed)
parmat_high <- get_grid("high", parmat0, pars_min_high, pars_max_high, pargrid, nsim, nfixed)
...
save(all, file = paste0("results/LHS_distgrid_array", array_id, "of", tot_array, "_", datestamp, ".RData"))
